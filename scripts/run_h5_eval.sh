...
MASK_SUBTYPING_RETAG_ANCHORS="${MASK_SUBTYPING_RETAG_ANCHORS:-0}"
...
if [[ "${MASK_SUBTYPING_RETAG_ANCHORS}" == "1" || "${MASK_SUBTYPING_RETAG_ANCHORS}" == "true" || "${MASK_SUBTYPING_RETAG_ANCHORS}" == "TRUE" ]]; then
  RESEARCH_ARGS+=(--mask_subtyping_retag_anchors)
fi
